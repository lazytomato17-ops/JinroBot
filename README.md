...
- Discordのスケジュールイベントで事前募集し、「興味あり」の参加者をロビーへ自動取り込み
...
| `/recruit`       | Discordイベントを作成して事前募集します。`minutes`で開始までの分数、`players`で予定人数を指定できます。 |
...
`/recruit` を使う場合は、Botロールと募集を作成するユーザーにDiscordの「イベントを作成」権限が必要です。既存導入済みサーバーでは、Botロールへこの権限を追加してください。
